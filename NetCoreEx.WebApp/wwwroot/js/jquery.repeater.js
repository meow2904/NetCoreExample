...
(function ($) {
    ...
    (function ($) {
        ...
        var createInput = function (fig, my) {
            ...
            self.get = function () {
                if (self.$().length > 0) {
                    var datetime;
                    var arrDate;
                    if (self.$()[0].className.includes("input-datetime")) {
                        datetime = self.$().val();
                        arrDate = datetime.substring(0, 10).split('/');
                        var arrTime = datetime.substring(11, 16);
                        return new Date((arrDate[2] + '-' + arrDate[1] + '-' + arrDate[0] + "T" + arrTime + ":00.000Z"));
                    }
                    else if (self.$()[0].className.includes("input-date")) {
                        datetime = self.$().val();
                        arrDate = datetime.substring(0, 10).split('/');
            ...
            self.set = function (newValue) {
                if (self.$().length > 0) {
                    ...
                    var arrTime;
                    if (self.$()[0].className.includes("input-datetime")) {
                        arrDate = newValue.substring(0, 10).split('-');
                        arrTime = newValue.substring(11, 16);
                        self.$().val(arrDate[2] + '/' + arrDate[1] + '/' + arrDate[0] + " " + arrTime);
                    }
                    else if (self.$()[0].className.includes("input-date")) {
                        arrDate = newValue.substring(0, 10).split('-');
                        arrTime = newValue.substring(11, 16);
                        self.$().val(arrDate[2] + '/' + arrDate[1] + '/' + arrDate[0]);
                    }
                    else
                        self.$().val(newValue);
                }
        ...
        var createInputCheckbox = function (fig) {
            ...
            self.get = function () {
                if (self.$().filter(':checked').length === 0) return false;
                return self.$().filter(':checked')[0].value === "1";
            };

            self.set = function (newValues) {
                self.$().prop('checked', newValues);
            };
    ...
    $.fn.repeater = function (fig) {
        ...
        $(this).each(function () {
                ...
                $items.each(function (index) {
                    ...
                    $filterNested($item.find('[name]'), repeaters)
                        .each(function () {
                            ...
                            var newName = groupName + '[' + index + '][' + name + ']' + ($input.attr('multiple') ? '[]' : '');
            ...
            setList = function (rows, functionLoad) {
                $items().remove();
                foreach(rows, addItem);
                functionLoad(getGroupName());
            };

            $filterNested($self.find('[data-repeater-create]'), fig.repeaters).click(function () {
                var end = $list.find('[data-repeater-item]').length - 1;
                if (fig.validateAdd(end, getGroupName())) {
                    addItem();
                    fig.loadAdd(end, getGroupName());
                }
            });

            $list.on('click', '[data-repeater-delete]', function () {
                var self = $(this).closest('[data-repeater-item]').get(0);
                var index = self.sectionRowIndex;
                hide.call(self, function () {
                    if (fig.removeItem(index, getGroupName())) {
                        $(self).remove();
                        setIndexes($items(), getGroupName(), fig.repeaters);
                    }
                });
